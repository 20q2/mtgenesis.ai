...
def createCardContent(prompt, card_data=None):
    ...
    try:
        ...
        if card_data:
            ...
            creature_context_guidance = ""
            if 'creature' in card_type:
                card_name = card_data.get('name', '')
                subtype = card_data.get('subtype', '')
                
                creature_context_guidance = " LEGENDARY CONSTRAINT: This is a legendary creature - structure abilities as follows: "
                ...
                if rarity == 'mythic':
                    creature_context_guidance += "Mythic legendary: Maximum 3-4 total abilities including keywords, triggered, and activated abilities. Make abilities feel unique and build-around worthy."
                elif rarity == 'rare':
                    creature_context_guidance += "Rare legendary: Maximum 3 total abilities including keywords. Focus on unique mechanics that feel special."
                elif rarity == 'uncommon':
                    creature_context_guidance += "Uncommon legendary: Maximum 2 total abilities including keywords. Simple but memorable effects."
                else:  # common
                    creature_context_guidance += "Common legendary: Maximum 1 total ability - either ONE keyword OR one simple triggered ability. Being legendary doesn't grant extra complexity."
                
                # Add name and subtype flavor guidance
                if card_name:
                    creature_context_guidance += f" This creature is named '{card_name}' - design abilities that reflect this specific character's identity, personality, and lore. Make the abilities feel unique to this individual."
                ...
                if subtype:
                    ...
                    if 'dragon' in subtype_lower:
                        creature_context_guidance += " As a Dragon, consider abilities like flying, dealing damage, treasure generation, or breath weapon effects."
                    elif 'angel' in subtype_lower:
                        creature_context_guidance += " As an Angel, consider abilities like flying OR vigilance (not both), lifegain, protection effects, flash, hexproof, or helping other creatures."
                    elif 'demon' in subtype_lower:
                        creature_context_guidance += " As a Demon, consider abilities like flying, menace, sacrifice effects, life drain, or punishing opponents."
                    elif 'beast' in subtype_lower:
                        creature_context_guidance += " As a Beast, consider abilities like trample, fighting other creatures, +1/+1 counters, or natural/primal effects."
                    elif 'wizard' in subtype_lower or 'mage' in subtype_lower:
                        creature_context_guidance += " As a Wizard/Mage, consider abilities related to spells, card draw, instant/sorcery synergies, or magical effects."
                    elif 'warrior' in subtype_lower or 'soldier' in subtype_lower:
                        creature_context_guidance += " As a Warrior/Soldier, consider abilities like first strike, vigilance, combat bonuses, or military tactics."
                    elif 'rogue' in subtype_lower or 'assassin' in subtype_lower:
                        creature_context_guidance += " As a Rogue/Assassin, consider abilities like deathtouch, unblockable, card advantage through sneaky means, or removal effects."
                    elif 'knight' in subtype_lower:
                        creature_context_guidance += " As a Knight, consider abilities like first strike, vigilance, protection effects, or honor-based abilities."
                    elif 'spirit' in subtype_lower:
                        creature_context_guidance += " As a Spirit, consider abilities like flying, phasing, graveyard interactions, or ethereal effects."
                    elif 'elemental' in subtype_lower:
                        creature_context_guidance += " As an Elemental, consider abilities related to basic lands, elemental forces, or effects that match your colors (fire=damage, water=card draw, etc.)."
                    elif 'vampire' in subtype_lower:
                        creature_context_guidance += " As a Vampire, consider abilities like lifelink, flying, life drain effects, or graveyard recursion."
                    elif 'zombie' in subtype_lower:
                        creature_context_guidance += " As a Zombie, consider abilities like deathtouch, graveyard recursion, sacrifice synergies, or undeath effects."
                    elif 'elf' in subtype_lower:
                        creature_context_guidance += " As an Elf, consider abilities like mana generation, creature synergies, forest/nature effects, or tribal bonuses."
                    elif 'goblin' in subtype_lower:
                        creature_context_guidance += " As a Goblin, consider abilities like haste, direct damage, artifact interactions, or chaotic/random effects."
                    elif 'human' in subtype_lower:
                        creature_context_guidance += " As a Human, consider versatile abilities that could represent leadership, innovation, adaptability, or cooperation with other creatures."
                    
                    creature_context_guidance += f" The subtype '{subtype}' should strongly influence the flavor and mechanics of the abilities."
                
                # Use flying restrictions from config
                
                if any(restricted_type in subtype_lower for restricted_type in flying_restricted_types):
                    creature_context_guidance += f" FLYING RESTRICTION: {subtype} creatures rarely have flying unless there's a specific magical reason (enchantment, spell effect, etc.). Consider grounded abilities like trample, first strike, vigilance, deathtouch, or reach instead."
                elif any(flying_type in subtype_lower for flying_type in flying_encouraged_types):
                    creature_context_guidance += f" FLYING ENCOURAGED: {subtype} creatures naturally fly and should strongly consider having flying as an ability."
            ...
            enhanced_prompt += f" The card costs {cmc} mana and should be {power_level}.{color_guidance}{type_specific_guidance}{creature_guidance}{creature_flying_guidance}{creature_context_guidance}{name_inspiration}{x_guidance}"
        ...
        if 'planeswalker' in card_type:
            ...
            # Add land type-specific mana production guidance
            land_guidance = " LAND FORMATTING: Focus on mana abilities and utility effects. Format activated abilities with proper costs. Example formats: '{T}: Add {W}', '{T}: Add one mana of any color', '{1}, {T}: Draw a card', '{T}: Target creature gets +1/+0 until end of turn'."
            
            # Add subtype-specific mana production guidance
            if subtype:
                subtype_lower = subtype.lower()
                
                # Basic land types
                if 'forest' in subtype_lower:
                    land_guidance += " FOREST CONTEXT: This is a Forest land - it should produce green mana. Consider abilities like '{T}: Add {G}' or green-themed effects involving creatures, +1/+1 counters, or nature."
                elif 'island' in subtype_lower:
                    land_guidance += " ISLAND CONTEXT: This is an Island land - it should produce blue mana. Consider abilities like '{T}: Add {U}' or blue-themed effects involving card draw, counterspells, or instant/sorcery synergies."
                elif 'mountain' in subtype_lower:
                    land_guidance += " MOUNTAIN CONTEXT: This is a Mountain land - it should produce red mana. Consider abilities like '{T}: Add {R}' or red-themed effects involving damage, haste, or aggressive strategies."
                elif 'plains' in subtype_lower:
                    land_guidance += " PLAINS CONTEXT: This is a Plains land - it should produce white mana. Consider abilities like '{T}: Add {W}' or white-themed effects involving lifegain, protection, or creature tokens."
                elif 'swamp' in subtype_lower:
                    land_guidance += " SWAMP CONTEXT: This is a Swamp land - it should produce black mana. Consider abilities like '{T}: Add {B}' or black-themed effects involving life loss, creature destruction, or graveyard interactions."
                
                # Special land types
                elif 'desert' in subtype_lower:
                    land_guidance += " DESERT CONTEXT: This is a Desert land - consider colorless mana production and desert-themed abilities like dealing damage to creatures or having enters-tapped restrictions."
                elif 'gate' in subtype_lower:
                    land_guidance += " GATE CONTEXT: This is a Gate land - typically produces two colors of mana but enters tapped. Consider multicolor mana production and gate synergies."
                elif 'cave' in subtype_lower:
                    land_guidance += " CAVE CONTEXT: This is a Cave land - consider colorless or specific color mana production with cave-themed abilities like providing shelter effects or underground synergies."
                elif 'lair' in subtype_lower:
                    land_guidance += " LAIR CONTEXT: This is a Lair land - consider producing mana of creature-type-specific colors and abilities that support particular creature types."
                elif 'shrine' in subtype_lower:
                    land_guidance += " SHRINE CONTEXT: This is a Shrine land - consider mana production tied to devotion, legendary matters, or spiritual/religious themed abilities."
                elif 'tower' in subtype_lower:
                    land_guidance += " TOWER CONTEXT: This is a Tower land - consider colorless mana production with abilities related to artifacts, spells, or reaching/tall effects."
                elif 'mine' in subtype_lower:
                    land_guidance += " MINE CONTEXT: This is a Mine land - consider colorless mana production with abilities related to artifacts, treasure, or resource extraction."
                elif 'sanctuary' in subtype_lower:
                    land_guidance += " SANCTUARY CONTEXT: This is a Sanctuary land - consider mana production with protective abilities, lifegain, or safe haven effects."
                
            enhanced_prompt += land_guidance
        else:
        ...
        enhanced_prompt += (" CREATIVITY AND UNIQUENESS REQUIREMENTS: 1) AVOID OVERUSED GENERIC ABILITIES: Never use these repetitive effects: 'Draw 3 cards', 'Draw a card', '{T}: Add one mana of any color', 'Tap: Create 1 mana', 'When this enters the battlefield, draw a card', 'Sacrifice this: Draw a card'. These are boring and overused. "
                           ...
                           "Example of GOOD cohesion: 'When this enters, create two 1/1 tokens' + '{T}, Sacrifice a creature: Draw a card' (token generation supports sacrifice). Example of BAD cohesion: 'Menace' + '{T}: Add mana' + 'Whenever a creature dies, gain 2 life' + 'Discard a card: Deal 1 damage' (random unrelated abilities). "
                           ...
                           "Your ENTIRE response should be ONLY abilities like: 'Reach', 'Vigilance', 'When this enters the battlefield...', '{T}: Add {G}', etc.\n"
                           ...
                           "Example of CORRECT output: '\"First Strike, trample\"' or '\"Flying, trample\" \"Whenever this creature attacks, gain 2 life\"' or '\"Prowess, trample\" \"{T}: Add one mana of any color\" \"When this enters the battlefield, create a 1/1 token\"'. "
                           ...
                           "Example of INCORRECT output with labels: 'Keywords: Deathtouch\\nTriggered Ability: When this enters, draw a card'. "
